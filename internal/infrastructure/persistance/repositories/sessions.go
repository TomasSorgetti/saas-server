...
func (r *sessionRepository) Create(ctx context.Context, session *entities.Session) error {
    query := `INSERT INTO sessions (user_id, access_token_hash, refresh_token_hash, expires_at, refresh_expires_at, is_valid)
              VALUES (?, ?, ?, ?, ?, ?)`
...
func (r *sessionRepository) FindByAccessTokenHash(ctx context.Context, accessTokenHash string) (*entities.Session, error) {
    query := `SELECT id, user_id, access_token_hash, refresh_token_hash, expires_at, refresh_expires_at, is_valid, created_at, updated_at
    ...
    var refreshExpiresAt sql.NullTime // Para manejar NULL en refresh_expires_at
    var updatedAt sql.NullTime        // Para manejar NULL en updated_at
    ...
    // Asignar valores de campos NULL
...
func (r *sessionRepository) Update(ctx context.Context, session *entities.Session) error {
    query := `UPDATE sessions SET access_token_hash = ?, refresh_token_hash = ?, expires_at = ?, refresh_expires_at = ?, is_valid = ?, updated_at = NOW()
              WHERE id = ?`
    _, err := r.db.ExecContext(ctx, query,
        session.AccessTokenHash,
        session.RefreshTokenHash,
